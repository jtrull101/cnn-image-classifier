...
def get_model() -> keras.Model:
    ...
    if not model:
        ...
        def find_model_in_dir(dir: str) -> None:
        ...
        for model_path in found_models:
            acc = int(model_path[model_path.find("%") - 2 : model_path.find("%")].replace("_", ""))
        ...
        model_accuracy, model_name = best

        print(f"loading model with accuracy = {model_accuracy}%")
        loaded_model = keras.models.load_model(model_name)
        assert isinstance(loaded_model, keras.Model), "Loaded model must be a keras.Model"
        model = loaded_model
    return model
