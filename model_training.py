...
def main():
    ...
    f.write(f"test run: {datetime.now().strftime('%m/%d/%Y, %H:%M:%S')}\n")
    f.write(f"accuracy percent,percent of data,batch size,history.params,history.history['acc'],history.history['loss'],elapsed_time\n")
    ...
    epochs =        [ 5, 10, 25, 50, 75, 100 ]                                      # 'random' scaling numbers
    batch_sizes =   [ 8, 16, 32, 64, 128 ]                                          # powers of 2
        ...
        for e in epochs:
            for b in batch_sizes:
                ...
                try:
                    ...
                    del history 
                    if acc >= 0.8:
                        model.save(f'models/alz_cnn_{e}_es_{b}_bs_{data_perc}_data_{acc_perc}_acc_{loss:.2f}_loss_{elapsed_time}_seconds.keras')
                        continue
