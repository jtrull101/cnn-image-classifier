...
class Trainer:
    ...
    def prepare_data(self) -> Tuple[np.ndarray, ...]:
        ...
        if self.config.data_percent < 1.0:
            ...
            X_val, y_val = self.data_loader.reduce_dataset(
                X_val, y_val, self.config.data_percent
            )
    ...
    def get_callbacks(self) -> List[tf.keras.callbacks.Callback]:
        ...
        if self.config.use_accuracy_threshold_stopping:
            acc_callback = AccuracyThresholdCallback(
    ...
    def save_model(
        ...
        if not force_save and acc < self.config.min_accuracy_to_save:
            print(f"Model accuracy {acc:.2%} below threshold {self.config.min_accuracy_to_save:.2%}, not saving")
            return None
    ...
    def log_results(
        ...
        if not log_file.exists():
            with open(log_file, 'w') as f:
                ...
                f.write("timestamp,accuracy,loss,data_percent,batch_size,learning_rate,epochs,elapsed_time\n")
    ...
    def run(
        ...
        try:
            ...
            # Plot training history
            if plot:
                plot_path = self.config.logs_dir / f"training_plot_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
